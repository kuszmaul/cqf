																																										 ...
																																										 uint64_t		 noverwrites)
{
	...
	for (i = 0; i < total_remainders; i++)
        ...
        qf->metadata->noccupied_slots += ninserts;
...
int64_t qf_resize_malloc(QF *qf, uint64_t nslots)
{
	...
	do {
		...
		qfi_next(&qfi);
		int ret = qf_insert(&new_qf, key, value, count, QF_KEY_IS_HASH);
		if (ret < 0) {
							...
							flags)
{
  ...
  if (ret == QF_NO_SPACE || ret > DISTANCE_FROM_HOME_SLOT_CUTOFF) {
    ...
    if (qf->runtimedata->auto_resize) {
      fprintf(stdout, "Resizing the CQF.\n");
      fprintf(stderr, "Here I am: %s:%d\n", __FILE__, __LINE__);
      fprintf(stderr, "contianer_resize=%p\n", qf->runtimedata->container_resize);
      if (qf->runtimedata->container_resize(qf, qf->metadata->nslots * 2) > 0)
      {
        fprintf(stderr, "Here I am: %s:%d\n", __FILE__, __LINE__);
        if (ret == QF_NO_SPACE) {
...
void qf_merge(const QF *qfa, const QF *qfb, QF *qfc)
{
	...
	do {
		if (keya < keyb) {
			qf_insert(qfc, keya, valuea, counta, QF_KEY_IS_HASH);
			qfi_next(&qfia);
			qfi_get_hash(&qfia, &keya, &valuea, &counta);
		}
		else {
			qf_insert(qfc, keyb, valueb, countb, QF_KEY_IS_HASH);
			qfi_next(&qfib);
			qfi_get_hash(&qfib, &keyb, &valueb, &countb);
		}
...
{
  int i;
  QFi *qfi_arr = malloc(nqf * sizeof(QFi));
  int smallest_idx = 0;
  ...
  uint64_t* keys = malloc(nqf * sizeof(uint64_t));
  uint64_t* values = malloc(nqf * sizeof(uint64_t));
  uint64_t* counts = malloc(nqf * sizeof(uint64_t));
  while (nqf > 1) {
    for (i=0; i<nqf; i++)
    ...
    do {
      ...
      qf_insert(qfr, keys[smallest_idx], values[smallest_idx],
                counts[smallest_idx], QF_KEY_IS_HASH);
  ...
  if (!qfi_end(&qfi_arr[0])) {
    ...
    do {
      ...
      qf_insert(qfr, key, value, count, QF_KEY_IS_HASH);
      qfi_next(&qfi_arr[0]);
      iters++;
